...
const AddGuideDetail = () => {
  ...
  async function fetchBlog() {
    try {
      ...
      setBlog(data.guide);
      setJsonDetail(data.guide.DetailBlog)
    } catch (error) {
  ...
  const handleImageChange = async (index, fieldPath, e) => {
    try {
      ...
      formData.append('guideimg', file);

      const response = await fetch(`${BaseURL}/uploadguideimage`, {
        method: 'POST',
        headers: {
          "AdminODSToken": sessionStorage.getItem('AdminODSToken')
        },
    ...
    <div className="w-[95%] m-auto bg-white flex flex-col gap-4 py-6 px-6 my-6 rounded-xl shadow-2xl">
      <div className="flex flex-row justify-between items-center">
        ...
        <div className="flex gap-2 flex-col md:flex-row">
          <a href={`/Preview-guide-Detail/${id}`} target='_blank'>
